...
Note: Due to the file size limitation from GitHub, some involved files might only contain the information of *TP53* and *DNMT3B* two genes, please modify [Gencode_converted_selected_exon_annotation.txt](./scripts/Translation_scripts/files/Gencode_converted_selected_exon_annotation.txt) and [Examples_visualization.sh](./scripts/Examples_visualization.sh) accordingly if other genes are desired.
...
And the `sample-subtype_match_table` is a tab-delimited file containing the subtype information for each input cell line, please refer to [BRCA_cell_lines.txt](./files/BRCA_cell_lines.txt) as an example.

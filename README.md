...
* **samples_BedGraph.bed**: A BED file that was directly converted from `samples_N2_R0_updated.gtf`, the transcript annotation file generated by ESPRESSO (GTF format). This BED file contains exon-level coordinates for each transcript isoform discovered from input samples.
* **samples_N2_R0_abundance_CPM_target_genes.esp**: a tab-delimited file describing the expression levels (in counts per million, CPM) for discovered transcript isoforms of target genes across input samples. This file was directly generated from `samples_N2_R0_abundance.esp` (an output file generated by ESPRESSO) by normalizing transcript isoform read counts for each input sample to CPM. The first three columns are: `transcript_ID`, `transcript_name`, and `gene_ID`. Additional columns correspond to input samples and show the CPM of the transcript isoform in a given sample.
* **samples_N2_R0_abundance_proportion_target_genes_reshaped_merge_others.txt**: a tab-delimited file with six columns where each row represents an individual transcript isoform of a target gene that was discovered in a given input sample. Each row has the following pieces of information: (i) gene ID, (ii) transcript ID, (iii) input sample ID, (iv) transcript isoform proportion in the given input sample, (v) tumor subtype associated with the given input sample, and (vi) hexadecimal color code associated with the tumor subtype. The top five transcript isoforms of a given target gene with the highest average transcript isoform proportion across all samples are represented individually, while remaining transcript isoforms are grouped together into an "Others" category.  
* **samples_N2_R0_abundance_CPM_target_genes_reshaped_merge_others.txt**: a tab-delimited file with six columns where each row represents an individual transcript isoform of a target gene that was discovered in a given input sample. Each row has the following pieces of information: (i) gene ID, (ii) transcript ID, (iii) input sample ID, (iv) CPM of the transcript isoform in the given input sample, (v) tumor subtype associated with the given input sample, and (vi) hexadecimal color code associated with the tumor subtype. The top five transcript isoforms of a given target gene with the highest average CPM across all samples are represented individually, while remaining transcript isoforms are grouped together into an "Others" category.  
...
```
python ./Generate_bar_structure_figure_example.py --gene [gene ID] --gene_name [gene symbol] --transcript [transcript isoform ID] \
  --abundance_CPM_original /path/to/samples_N2_R0_abundance_CPM_target_genes.esp --name_col 4 \
  --abundance_proportion /path/to/samples_N2_R0_abundance_proportion_target_genes_reshaped_merge_others.txt \
  --abundance_CPM /path/to/samples_N2_R0_abundance_CPM_target_genes_reshaped_merge_others.txt \
  --bedgraph /path/to/samples_BedGraph.bed --out_dir /path/to/output/directory --order yes
```
...
```
# Visualize transcript isoforms of TP53
python ./Generate_bar_structure_figure_example.py --gene ENSG00000141510 --gene_name TP53 --transcript ESPRESSO:chr17:1864:802 \
  --abundance_CPM_original ./Example_res/samples_N2_R0_abundance_CPM_target_genes.esp --name_col 4 \
  --abundance_proportion ./Example_res/samples_N2_R0_abundance_proportion_target_genes_reshaped_merge_others.txt \
  --abundance_CPM ./Example_res/samples_N2_R0_abundance_CPM_target_genes_reshaped_merge_others.txt \
  --bedgraph ./Example_res/samples_BedGraph.bed --out_dir ./Example_res --order yes

# Visualize transcript isoforms of DNMT3B
python ./Generate_bar_structure_figure_example.py --gene ENSG00000088305 --gene_name DNMT3B --transcript ENST00000348286 \
  --abundance_CPM_original ./Example_res/samples_N2_R0_abundance_CPM_target_genes.esp --name_col 4 \
  --abundance_proportion ./Example_res/samples_N2_R0_abundance_proportion_target_genes_reshaped_merge_others.txt \
  --abundance_CPM ./Example_res/samples_N2_R0_abundance_CPM_target_genes_reshaped_merge_others.txt \
  --bedgraph ./Example_res/samples_BedGraph.bed --out_dir ./Example_res --order yes
...
These commands are contained in a Bash wrapper script [Examples_visualization.sh](./scripts/Examples_visualization.sh), which can be executed as follows:
1. Enter the [scripts](./scripts/) folder.
2. Run the following command: `bash Examples_visualization.sh`

Plots generated by our script (in PDF format) can be found in the folder [Example_res](./scripts/Example_res/).
...
* A tab-delimited file containing transcript isoform proportions for discovered transcripts across input samples. This file can be directly generated from `samples_N2_R0_abundance.esp` (an output file generated by ESPRESSO) by dividing the read count of a transcript isoform by the total read count for the corresponding gene (i.e., sum of read counts over all transcript isoforms discovered for the gene) for each input sample. The first three columns are: `transcript_ID`, `transcript_name`, and `gene_ID`. Additional columns correspond to input samples and show the proportion of the transcript isoform in a given sample. Please refer to the file [samples_N2_R0_abundance_proportion_target_genes.txt](./scripts/Example_res/samples_N2_R0_abundance_proportion_target_genes.txt) as an example. 
* A tab-delimited file mapping each input sample ID to a tumor subtype. Each row represents an input sample and has three columns: (i) a sample ID, (ii) a cell line ID, and (iii) a tumor subtype. Please refer to the file [BRCA_cell_lines.txt](./files/BRCA_cell_lines.txt) as an example.
...
script arguments:
    ...
    -i /path/to/isoform_proportion_matrix               tab-delimited file containing transcript isoform proportions for discovered transcripts
...
script arguments:
    ...
    -i /path/to/input/GTF                               path to GTF file containing transcript-level and exon-level coordinates for two transcript isoforms
...
script arguments:
    ...
    --genome_version                                    human reference genome build
                                                        ("GRCH37" or "GRCH38")

    --ref_gtf                                           path to gtf file containing reference transcript annotations

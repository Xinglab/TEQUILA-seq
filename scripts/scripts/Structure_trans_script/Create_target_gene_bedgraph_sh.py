...
object_path = '/'.join(os.path.abspath(CPM_inf_name).split('/')[0:-1])
if len(sys.argv) > 4:
...
for each_sample in sample_list:
	cmd = 'python '+dire_path+'/Generate_bedgraph_for_each_gene.py '+each_sample+' '+target_gene_ID+' '+CPM_inf_name+' '+CPM_cut_off+' '+object_path+' '+sample_bedgraph+' '+anno_gtf_file+'\n'
	outf.write(cmd)
	print (cmd)

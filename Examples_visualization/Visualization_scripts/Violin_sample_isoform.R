...
#gene_data <- gene_data[order(gene_data$Subtype, decreasing=FALSE),]
#gene_data$Sample <- factor(gene_data$Sample, levels=as.vector(unique(gene_data$Sample)))
#gene_data$Transcript_ID <- factor(gene_data$Transcript_ID, levels=(as.vector(unique(gene_data$Transcript_ID))))
#gene_data

#colorCount = length(unique(gene_data$Subtype))

...
CPM_inf_name = args[6]
order_flag = args[7]
#subtype_order <- c('Myc_amp','Myc_normal','Cell_line')
#subtype_order <- c('Melanocytic','Neural crest like','Transitory','Undifferentiated','Tissue')
subtype_order <- c('Basal_B','Basal_A','HER2_Amp','Luminal')
...
uniprot_inf_name = '/mnt/isilon/xing_lab/aspera/xuy/snakemake_ESPRESSO_reference/pipeline_test/scripts_example_figure/Gencode_v39_canonical_isoform.txt'
...
basic_inf_name = '/mnt/isilon/xing_lab/aspera/xuy/snakemake_ESPRESSO_reference/pipeline_test/scripts_example_figure/gencode.v34lift37.annotation_basic_trans.txt'
...
#display_table = data.frame(lapply(isoform_reorder_table, function(x){gsub("@Melanoma","",x)}), stringsAsFactors=F)
...
#trans_color_original_list <- c('#7D4655','#B8604D','#E0BBA4','#86a65d','#438B80','#413E60')
#trans_color_original_list <- c('#a90f0f','#458baf','#cde1eb','#87b6cf','#295369','#94a3aa')
#trans_color_original_list <- c('#FC402D','#08589e','#2b8cbe','#4eb3d3','#7bccc4','#94a3aa')
#trans_color_original_list <- c('#FC402D','#2171b5','#4292C6','#6baed6','#9ecae1','#97a1a6')
...
#f_fig <- f_fig + scale_colour_manual(name="Subtype",values=c("#FF0018", "#fcb453", "#02a620","#0000F9","#86007D"),labels=c("Basal A","Basal B","HER2_Amp","Luminal","Non-tumorigenic"),breaks=c("Basal_A","Basal_B","HER2_Amp","Luminal","Non-tumorigenic"))
f_fig <- f_fig + scale_colour_manual(name="Group",values=c("#FF0018","#e68e19","#02a620","#0000F9","#86007D"))
#f_fig <- f_fig + scale_colour_manual(name="Subtype",values=c("#FF0018","#e68e19","#02a620","#0000F9","#86007D"),labels=c('Melanocytic','Neural crest like','Transitory','Undifferentiated','Tissue'))
f_fig <- f_fig + geom_bar(stat="identity", position = position_stack(vjust = 1, reverse = FALSE), width=0.9)
...
#png(file=outf_name, width=10, height=4,units='in',res=300)
#f_fig
#dev.off()
#ggsave(outf_pdf_name, plot=f_fig, width=10, height=4, dpi=300)
...
gene_data_exp$Subtype <- factor(gene_data_exp$Subtype, levels=subtype_order)
...
#f_fig_exp <- f_fig_exp + scale_colour_manual(name="Subtype",values=c("#FF0018", "#fcb453", "#02a620","#0000F9","#86007D"),labels=c("Basal A","Basal B","HER2_Amp","Luminal","Non-tumorigenic"),breaks=c("Basal_A","Basal_B","HER2_Amp","Luminal","Non-tumorigenic"))
f_fig_exp <- f_fig_exp + scale_colour_manual(name="Group",values=c("#FF0018","#e68e19","#02a620","#0000F9","#86007D"))
#f_fig_exp <- f_fig_exp + scale_colour_manual(name="Subtype",values=c("#FF0018","#e68e19","#02a620","#0000F9","#86007D"),labels=c('Melanocytic','Neural crest like','Transitory','Undifferentiated','Tissue'))
f_fig_exp <- f_fig_exp + geom_bar(stat="identity", position = position_stack(vjust = 1, reverse = TRUE), width=0.9)
...
#f_fig_exp <- f_fig_exp + scale_y_continuous(label = unit_format(unit = "K",sep="",accuracy=0.1))
#f_fig <- f_fig + scale_y_continuous(labels = scales::number_format(accuracy = 0.1), limits=c(0,15))
...
#### combined plots ####
legend_a <- get_legend(f_fig + guides(fill = guide_legend(reverse = FALSE, ncol=1, title.position='top', label.theme = element_text(size = font_size-1),keywidth=1, keyheight=1, order=1)) + guides(color = guide_legend(reverse = FALSE, ncol=1, title.position='top', label.theme = element_text(size = font_size-1), keywidth=1, keyheight=1, order=2)) + theme(legend.position='right'))
box_plot <- plot_grid(f_fig_exp, f_fig, align = "v", ncol=1, rel_heights = c(1,1.8))
combined_plot <- plot_grid(box_plot, legend_a, nrow=1, align='h', axis='t', rel_widths=c(1,0.25))
...
png(file=outf_exp_name, width=def_width, height=5, units='in',res=1500)
combined_plot
dev.off()
ggsave(outf_pdf_exp_name, plot=combined_plot, width=def_width, height=5, dpi=300)
